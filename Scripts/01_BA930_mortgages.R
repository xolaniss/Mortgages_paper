...
mortgage_lending_tbl <- 
  ...
  mutate(Description = str_replace_all(Description, " fixed", "fixed")) %>%
  mutate(Description = str_replace_all(Description, 
                                       "fixed rate- corporate sector mortgage advances" , 
                                       "Mortgage advances-  fixed rate- corporate sector")) %>%
  mutate(Description = str_replace_all(Description,
                                       "fixed rate- household sector mortgage advances",
                                       "Mortgage advances-  fixed rate- household sector")) %>% 
  separate(Description, into = c("Delete", "Rate type", "Sector"), sep = "- ") %>% 
  dplyr::select(-Delete) %>% 
